...
in this repository.  Normally, I rely on automatic line endings and use the
platform line endings on whatever platform I'm developing on.  That doesn't
work well for this repo, because the PyPI package gets the line endings from
whatever platform the package was released on, and Windows line endings cause
problem for the Debian packaging process.  So, the `.gitattributes` file forces
most files (except for a few platform-specific files like `.ps1`) to have UNIX
line endings.  

This generally works ok, except for the [`docs/requirements.txt`](docs/requirements.txt) file 
generated by Poetry.  Unfortunately, Poetry generates this file with the
platform line endings, and you can't override it that behavior.  Even with sane
configuration in `.gitattributes`, you sometimes still get spurious
differences, where Git says that a file has changed but then `git diff` shows
an empty result.  So, the `run` script and the pre-commit hooks normalize the
line endings manually, using [`dos2unix.py`](dos2unix.py).  I wish there were a
standard way to do this in Poetry or in Python, so I didn't have to carry this
script around.
